...
//###################################################################################################################
/*
 * DMA version ReadBytes()
 * This requires, in addition to the standard requirements:
 * 1. a DMA channel (peripheral to memory) configured for _W25QXX_SPI
 * 2. an interrupt for the DMA channel that calls HAL_DMA_IRQHandler()
 * 3. a callback (HAL_SPI_RxCpltCallback) that un-asserts the CS pin by setting it high,
 */

//void W25qxx_ReadBytes_DMA(uint8_t* pBuffer, uint32_t ReadAddr, uint32_t NumByteToRead)
//{
//	/* acquire lock */
//	while(w25qxx.Lock==1)
//		W25qxx_Delay(1);
...
//
//	/* receive data */
////	HAL_SPI_TransmitReceive_DMA(&_W25QXX_SPI, (uint8_t*)0x08000000, pBuffer, NumByteToRead);
//	HAL_SPI_Receive_IT(&_W25QXX_SPI,pBuffer,NumByteToRead);
//
//	/* let stuff finish in the background */
//}
//###################################################################################################################
